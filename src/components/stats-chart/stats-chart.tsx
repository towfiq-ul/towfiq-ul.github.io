...
function calculateTechExperience() {
    ...
    const phpStart = new Date(2016, 4);
    const dbStart = new Date(2018, 4);
    const devopsStart = new Date(2019, 8);
    const javaStart = new Date(2019, 11);
    const cloudStart = new Date(2020, 3);
    ...
    return [
        {
            name: "Java/Spring",
            years: Number((javaExp.years + javaExp.months / 12).toFixed(1)),
            color: "var(--color-accent-9)"
        },
        {
            name: "PHP/Laravel",
            years: Number((phpExp.years + phpExp.months / 12).toFixed(1)),
            color: "var(--color-accent-7)"
        },
        {
            name: "Cloud/AWS",
            years: Number((cloudExp.years + cloudExp.months / 12).toFixed(1)),
            color: "var(--color-accent-8)"
        },
        {
            name: "Databases",
            years: Number((dbExp.years + dbExp.months / 12).toFixed(1)),
            color: "var(--color-accent-10)"
        },
        {
            name: "DevOps",
            years: Number((devopsExp.years + devopsExp.months / 12).toFixed(1)),
