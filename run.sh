...
#                      workers/.dev.vars for secrets) and the frontend
...
if [[ "${1:-}" == "prod" ]]; then
    ...
    VITE_AI_PROXY_URL="$PROD_PROXY_URL" npm --prefix "$ROOT_DIR" run start
    exit 0
fi

if [[ ! -f "$ROOT_DIR/workers/.dev.vars" ]]; then
    echo "Missing workers/.dev.vars (needs AI_API_KEY and AI_BASE_URL for local wrangler dev)." >&2
...
(cd "$ROOT_DIR/workers" && npx wrangler dev --port "$WORKER_PORT") &
WRANGLER_PID=$!

echo "Starting frontend, pointed at the local worker..."
VITE_AI_PROXY_URL="http://localhost:$WORKER_PORT" npm --prefix "$ROOT_DIR" run start
